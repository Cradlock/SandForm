...
> Обязательно должны быть в файле `modules.json` (Должен быть рядом с исполняемой 
программой):
```
{
  "modules":[
...
- `modules`: Обший список всех модулей

- `id`:Строка обозначаюшее смысл модуля

- `path`:Строка указывающая путь и название

- `required`:Булевое значение чтобы обозначить важность модуля


## Слинкованные динамически с `core`
> Некоторые модули изначально слинкованы с core.dll и их нельзя тупо заменить.
> Они не загружаются динамически
- `app`: Сама точка входа тоже динамически слинкована с core.dll
...
## Зависимости
- `nlohmann/json`:Модуль для работы с json файлами

- `boost.DLL`:Модуль для загрузки динамических библиотек на разных ОС.
